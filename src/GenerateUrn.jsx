...
import { color_palette, importantcolor, othercolor } from './randomizedParameters';


const GenerateUrn = ({ ids, urnColorID, urnLetter, prob, drawn, 
  scoreSetterImportant, scoreSetterOther, setUrnCounter, phase, 
   ballColors, score, setScoreColor, urnID, scoreSetter }) => {

  
  //the number of points associated with the ball we draw
  let power = drawn;
  let color = phase === "instructions" ? ballColors[1] : color_palette[urnID - 1];

  //the function that updates the score. This needs to be the function that updates
  //the number of red balls, or the one updating the number of blue balls,
  //depending on the color of the current urn
  const setterFunction = phase === "instructions" ? scoreSetter : 
  color === importantcolor ? scoreSetterImportant :
  color === othercolor ? scoreSetterOther : "NA";
