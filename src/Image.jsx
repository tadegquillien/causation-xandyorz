...
const Image = (props) => {
    ...
    return props.shuffledUrnIds.map((i) => {
        ...
        //display the urn
        return (
            <GenerateUrn ids={circle_ids} urnColorID={colors[i - 1]} 
            urnLetter={chosenLetter} prob={PROBS[i - 1]} drawn={world[i - 1]}
                scoreSetterImportant={props.scoreSetterImportant}
                scoreSetterOther={props.scoreSetterOther}
                 setUrnCounter={props.setUrnCounter} phase={props.phase}
                ballColors={props.ballColorsList[i - 1]} 
                scoreSetter={props.scoreSetter}
                setImportantScoreColor={props.setImportantScoreColor}
                setOtherScoreColor={props.setOtherScoreColor}
                scoreImportantColor={props.scoreImportantColor}
                scoreOtherColor={props.scoreOtherColor}
                 urnID={i}/>
        )
    }
    )
